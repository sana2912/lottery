...
- Run `bun run db:compute-analysis` if you want to verify the shared analysis snapshot path for analytics, patterns, and calendar.
- Run `bun run db:compute-analysis -- --prizeType=TWO_DIGIT --scope=ALL_TIME --windowPreset=50` if you want to refresh one all-time context.
- Run `bun run db:compute-analysis -- --prizeType=FIRST --scope=MONTH --month=5 --windowPreset=100` if you want to refresh one month-scoped context.
